resource "aws_security_group_rule" "ingress_collections_rehydration_metastore_v2" {
  description              = "Allow mysql traffic to Aurora RDS from collections-rehydration"
  from_port                = 3306
  protocol                 = "tcp"
...
resource "aws_security_group_rule" "egress_collections_rehydration_metastore_v2" {
  description              = "Allow mysql traffic to Aurora RDS from collections-rehydration"
  from_port                = 3306
  protocol                 = "tcp"
