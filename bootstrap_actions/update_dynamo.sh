...
(
    ...
    function log_wrapper_message() {
        log_dataworks_aws_collections_rehydration_message "$${1}" "update_dynamo.sh" "$${PID}" "$${@:2}" "Running as: ,$USER"
    }
  ...
  DATE=$(date '+%Y-%m-%d')
  DATA_PRODUCT="DataWorksCollectionsRehydration"
...
) >> /var/log/dataworks-aws-collections-rehydration/update_dynamo_sh.log 2>&1
