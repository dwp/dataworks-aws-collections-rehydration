...
(
    ...
    function log_wrapper_message() {
        log_dataworks_aws_collections_rehydration_message "$${1}" "emr-setup.sh" "$${PID}" "$${@:2}" "Running as: ,$USER"
    }
    ...
    export NO_PROXY="$FULL_NO_PROXY"
    export dataworks_aws_collections_rehydration_LOG_LEVEL="${dataworks_aws_collections_rehydration_LOG_LEVEL}"
...
EOF
    ...
    --jks-only true >> /var/log/dataworks-aws-collections-rehydration/acm-cert-retriever.log 2>&1
    ...
    --private-key-alias "${private_key_alias}" \
    --truststore-aliases "${truststore_aliases}" \
    --truststore-certs "${truststore_certs}"  >> /var/log/dataworks-aws-collections-rehydration/acm-cert-retriever.log 2>&1 # No sudo needed to write to file, so redirect is fine
...
) >> /var/log/dataworks-aws-collections-rehydration/emr-setup.log 2>&1
