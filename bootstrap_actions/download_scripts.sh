...
sudo mkdir -p /var/log/dataworks-aws-collections-rehydration
sudo mkdir -p /opt/emr
sudo mkdir -p /opt/shared
sudo mkdir -p /var/ci
sudo chown hadoop:hadoop /var/log/dataworks-aws-collections-rehydration
...
(
    ...
    function log_wrapper_message() {
        log_dataworks_aws_collections_rehydration_message "$${1}" "download_scripts.sh" "$${PID}" "$${@:2}" "Running as: ,$USER"
    }
...
)  >> /var/log/dataworks-aws-collections-rehydration/download_scripts.log 2>&1
