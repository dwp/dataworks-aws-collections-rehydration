...
(
    ...
    function log_wrapper_message() {
        log_dataworks_aws_collections_rehydration_message "$${1}" "status_metrics.sh" "$${PID}" "$${@:2}" "Running as: ,$USER"
    }
  ...
  push_metric() {
    ...
    cat << EOF | curl --data-binary @- "http://${collections_rehydration_pushgateway_hostname}:9091/metrics/job/DataWorksCollectionsRehydration"
                collections_rehydration_status{component="DataWorksCollectionsRehydration",snapshot_type="$SNAPSHOT_TYPE", export_date="$EXPORT_DATE", cluster_id="$CLUSTER_ID",correlation_id="$CORRELATION_ID"} $1
EOF
...
) >> /var/log/dataworks-aws-collections-rehydration/status_metrics.log 2>&1
