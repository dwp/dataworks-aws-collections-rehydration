...
(
    ...
    function log_wrapper_message() {
        log_dataworks_aws_collections_rehydration_message "$${1}" "config_hcs.sh" "$${PID}" "$${@:2}" "Running as: ,$USER"
    }
...
)   >> /var/log/dataworks_aws_collections_rehydration/config_hcs.log 2>&1